...
class ConfigVFDDisplay(Screen, ConfigListScreen):
	...
	def keySave(self):
		global DisplayType
		if DisplayType == 12:
			print("[UFSVFD] set LED brightness", config.plugins.vfdicon.ledbright.value)
			b = str(config.plugins.vfdicon.ledbright.value)
			Console().ePopen("fp_control -led" + b)
		...
		print("[UFSVFD] set text scroll", config.plugins.vfdicon.textscroll.value)
		if config.plugins.vfdicon.textcenter.value == "1":
			evfd.getInstance().vfd_set_CENTER(True)
		else:
			evfd.getInstance().vfd_set_CENTER(False)
		print("[UFSVFD] set text centering", config.plugins.vfdicon.textcenter.value)
		main(self)
...
class VFDIcons:
	def __init__(self, session):
		...
		Console().ePopen("fp_control -led" + b)
		global DisplayType
		print('[UFSVFD] Hardware displaytype:', DisplayType)
		print('[UFSVFD] VFD displaytype     :', DisplayTypevfd)
		if DisplayType == 12:
		...
		if config.plugins.vfdicon.textscroll.value is not None:
			...
			evfd.getInstance().vfd_set_SCROLL(1)
		print("[UFSVFD] Set text centering option")
		if config.plugins.vfdicon.textcenter.value == "1":
			evfd.getInstance().vfd_set_CENTER(True)
		else:
			evfd.getInstance().vfd_set_CENTER(False)
		print('[UFSVFD] End initialisation')

	def __evStart(self):
		print('[UFSVFD] __evStart')
		self.__evSeekableStatusChanged()

	def __evUpdatedEventInfo(self):
		print('[UFSVFD] __evUpdatedEventInfo')
#		... and do nothing else

	def UpdatedInfo(self):
		print('[UFSVFD] __evUpdatedInfo')
...
#								servicename = ServiceReference(service).getServiceName().replace('\xc2\x87', '').replace('\xc2\x86', '').ljust(63)
	...
	def onLeaveStandby(self):
		...
		print("[UFSVFD] minute timer started")
		if DisplayType == 12:
			evfd.getInstance().vfd_set_brightness(config.plugins.vfdicon.contrast.value)
			print("[UFSVFD] set brightness", config.plugins.vfdicon.contrast.value)
			self.timerEvent()
			b = str(config.plugins.vfdicon.ledbright.value)
			Console().ePopen("fp_control -led" + b)
			if config.plugins.vfdicon.showicons.value == "all":
			...
			print("[UFSVFD] set icons on Leave Standby")
	...
	def onEnterStandby(self, configElement):
		...
		if DisplayType == 12:
			...
			print("[UFSVFD] set display & icons on Enter Standby")
			...
			print("[UFSVFD] set standby brightness", config.plugins.vfdicon.stbcontrast.value)
		...
		if (config.plugins.vfdicon.stbshow.value == "time" or config.plugins.vfdicon.stbshow.value == "time_date"):
			...
			print("[UFSVFD] second timer started")
	...
	def displayHddUsed(self):
		...
		if config.plugins.vfdicon.showicons.value == "all":
			if self.mount == None:
				...
				print("[UFSVFD] HDD mount point:", self.mount)
		else:
