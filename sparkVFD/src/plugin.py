...
class ConfigVFDDisplay(Screen, ConfigListScreen):
	...
	def keySave(self):
		...
		if DisplayTypevfd == 8:
			Console().ePopen("fp_control -b " + str(config.plugins.vfdicon.contrast.value))
			print("[sparkVFD] set brightness", config.plugins.vfdicon.contrast.value)
		...
		if DisplayTypevfd == 8:
			...
			print("[sparkVFD] set text centering", config.plugins.vfdicon.textcenter.value)
		main(self)
...
#						if ts and ts.isTimeshiftEnabled() > 0:
	...
	def onLeaveStandby(self):
		...
		if DisplayType == 8:
			...
			print("[sparkVFD] set icons on Leave Standby")
...
#				Console().ePopen("fp_control -L 0")
			...
			print("[sparkVFD] set standby brightness", config.plugins.vfdicon.stbcontrast.value)
			...
			if config.plugins.vfdicon.hddicons.value != "no":
				if self.mount == None:
					...
					if hddUsed != used: # if previous size different
						...
						print("[sparkVFD] HDD mount point:", self.mount, ", used icons:", used / 10)
		else:
			self.displayHddUsedOff()
