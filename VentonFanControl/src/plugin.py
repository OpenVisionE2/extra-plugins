...
def Plugins(**kwargs):
	...
	if path.exists("/proc/stb/fp/fan"):
		...
		return [PluginDescriptor(name=_("Venton Fan Control"), description=_("switch Fan On/Off"), where=PluginDescriptor.WHERE_MENU, fnc=FanSetup),
					PluginDescriptor(name="Venton Fan Control", description="", where=PluginDescriptor.WHERE_SESSIONSTART, fnc=startup)]
	return []
