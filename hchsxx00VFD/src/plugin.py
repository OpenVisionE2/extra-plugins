...
from enigma import iPlayableService, iServiceInformation, iRecordableService, eTimer, evfd, iFrontendInformation
...
class ConfigVFDDisplay(Screen, ConfigListScreen):
	...
	def keySave(self):
		...
		if DisplayType == 22:
			Console().ePopen("fp_control -b " + str(config.plugins.vfdicon.contrast.value))
			print("[hchsxx00VFD] set brightness", config.plugins.vfdicon.contrast.value)
		...
		print("[hchsxx00VFD] set text scroll", config.plugins.vfdicon.textscroll.value)
		if config.plugins.vfdicon.textcenter.value == "1":
			evfd.getInstance().vfd_set_CENTER(True)
		else:
			evfd.getInstance().vfd_set_CENTER(False)
		print("[hchsxx00VFD] set text centering", config.plugins.vfdicon.textcenter.value)
		main(self)
...
class VFDIcons:
	def __init__(self, session):
		...
		global DisplayType
		global hddUsed
		print('[hchsxx00VFD] Hardware displaytype:', DisplayType)
		print('[hchsxx00VFD] VFD displaytype     :', DisplayTypevfd)
		if DisplayType == 22:
		...
		print('[hchsxx00VFD] Set text scrolling option')
		if config.plugins.vfdicon.textscroll.value is not None:
			evfd.getInstance().vfd_set_SCROLL(int(config.plugins.vfdicon.textscroll.value))
		else:
			evfd.getInstance().vfd_set_SCROLL(1)
		print("[hchsxx00VFD] Set text centering option")
		if config.plugins.vfdicon.textcenter.value == "1":
			evfd.getInstance().vfd_set_CENTER(True)
		else:
			evfd.getInstance().vfd_set_CENTER(False)
		print('[hchsxx00VFD] End initialisation')

	def __evStart(self):
		print('[hchsxx00VFD] __evStart')
		self.__evSeekableStatusChanged()

	def __evUpdatedEventInfo(self):
		print('[hchsxx00VFD] __evUpdatedEventInfo')
#		... and do nothing else

	def UpdatedInfo(self):
		print('[hchsxx00VFD] __evUpdatedInfo')
...
#			self.timer.start(60000, False) # start minute timer
	...
	def onLeaveStandby(self):
		...
		if DisplayType == 22:
			...
			if config.plugins.vfdicon.showicons.value == "all":
				...
				hddUsed = -1  #force hdd display
				self.displayHddUsed()
			print("[hchsxx00VFD] set icons on Leave Standby")
	...
	def onEnterStandby(self, configElement):
		...
		if DisplayType == 8:
			...
			print("[hchsxx00VFD] set standby brightness", config.plugins.vfdicon.stbcontrast.value)
		...
		self.standby = True
		print("[hchsxx00VFD] set display & icons on Enter Standby")
	...
	def SetMount(self):
		if config.plugins.vfdicon.hddicons.value == "all mounts":
			...
			if not self.mount or self.dir != dir:
				if not self.mount:
					self.dir = dir
#					print("[hchsxx00VFD] SetMount", dir)
			...
			try:
				...
				if self.mount:
					try:
						...
						self.mount = None
#			print("[hchsxx00VFD] Mountpoint       :", self.mount)
#			if self.mount != None:
#				print("[hchsxx00VFD] Total blocks     :", f.f_blocks)
#				print("[hchsxx00VFD] Free blocks      :", f.f_bavail)
#			if f.f_blocks != 0:
#				print("[hchsxx00VFD] Free/Used space  :", f.f_bavail * 100 / f.f_blocks, "/", (f.f_blocks - f.f_bavail) * 100 / f.f_blocks, "%")
		if self.mount:
			if f.f_blocks == 0:
				return 0
			else:
				print("[hchsxx00VFD] HDD free space:", 100 - ((f.f_blocks - f.f_bavail) * 100 / f.f_blocks), "%")
			...
			if config.plugins.vfdicon.hddicons.value != "no":
				if self.mount == None:
					...
					if hddUsed != used: # if previous size different
						...
						print("[hchsxx00VFD] HDD mount point:", self.mount, ", used icons:", used / 10)
		else:
			self.displayHddUsedOff()
...
def Plugins(**kwargs):
	l = [PluginDescriptor(
		...
		PluginDescriptor(
		name=_("hchsxx00VFD"),
		description=_("VFD icons for Homecast HC8100/9000"),
		where=PluginDescriptor.WHERE_SESSIONSTART,
		fnc=main)]
	if config.plugins.vfdicon.extMenu.value:
		l.append(PluginDescriptor(
			name=_("hchsxx00VFD"),
			description=_("VFD display configuration for Homecast HC8100/9000"),
